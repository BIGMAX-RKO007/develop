...
from t_shop_order
group by 
grouping sets(dt, store_id, (dt, store_id));
```
...
不能通过NULL判断分组维度，是不准确的。

grouping(store_id)  返回0，是当前行正在按照store_id进行分组；返回1，相反。

![image-20211220173547004](笔记2.DWS-Presto - 答案.assets/image-20211220173547004.png)
...
cart_count_amount as (
   select
      ...
      sum(buy_num * coalesce(g.goods_promotion_price,0)) as cart_amount,
...
![行拼接和列拼接](笔记2.DWS-Presto - 答案.assets/行拼接和列拼接-16400725489271.png)
